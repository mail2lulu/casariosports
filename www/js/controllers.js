...
angular.module('starter.controllers', [])
  ...
  .controller('ProfileCtrl', function ($scope, $state, $stateParams, $timeout, ionicMaterialMotion, ionicMaterialInk, $location, $ionicModal, $ionicHistory) {
    try {
      ...
      function getFilteredPlayers() {
        var ref = firebase.database().ref(`schools/${schoolFolder}/${subFolder}`);
        console.log('2 schoolFolder: ', schoolFolder);
        // ref.orderByChild("division").equalTo("C").on("child_added", function (snapshot) {
        // ref.orderByChild("gradeName").equalTo("Class 5").on("child_added", function (snapshot) {
        // ref.orderByChild("time").equalTo(1499743055359).on("child_added", function (snapshot) {
        ref.once("value", function (snapshot) {
          console.log('snapshot.val: ', snapshot.val());
          if (snapshot.val()) {
            console.log('origin Object.keys(snapshot.val()).length: ', Object.keys(snapshot.val()).length);
          }
        });
      }
  ...
  .controller('CollectionCtrl', function ($scope, $stateParams, $ionicPopup, $timeout, ionicMaterialMotion, ionicMaterialInk, $ionicHistory, $state) {
    console.log('inside Collection Ctrl: ');
  ...
  .controller('AdminCtrl', function ($scope, $stateParams, $ionicPopup, $timeout, ionicMaterialMotion, ionicMaterialInk, $ionicHistory, $state) {
    ...
    function updatePlayerData(player, callee) {
      console.log('update PlayerData player: ', player);
      var userData = {
        ccl2017Payment: myAppConfig.user.uid,
        ccl2017PaidTo: myAppConfig.user.displayName
      }
      var res2 = crsapp.firebase.updateUserData(userData, player.uid).then(ref => {
        console.log('User updated data ref:', ref);
      })
     ...
     * @param {*} callee 
     */
    function showPaymentConfirm(player, callee) {
      var thePlayer = player;
      var confirmPopup = $ionicPopup.confirm({
        title: 'CCL 2017',
        template: 'Are you sure got payment for CCL 2017?',
        buttons: [{
            text: 'No',
            onTap: function (e) {
              console.log('thePlayer 1: ', thePlayer);
          ...
          },
          {
            ...
            onTap: function (e) {
              console.log('thePlayer 2: ', thePlayer);
      ...
      confirmPopup.then(function (res) {
        ...
        console.log('thePlayer 3: ', thePlayer);
        if (res) {
          console.log('You are sure payment');
          updatePlayerData(thePlayer, "confirm Popup")

        } else {
          console.log('You are not sure payment');
    ...
    $scope.updatePayment = function (player, me, evt) {
      console.log('update Payment player: ', player);
      //   console.log('evt: ', evt);
      //   console.log('updatePayment, me: ', player, me);
      // myAppConfig.currUser = player;
      showPaymentConfirm(player, "payment change click")
      evt.stopPropagation();
      evt.preventDefault();
    }
